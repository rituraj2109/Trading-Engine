...
class TechnicalAnalysis:
    ...
    def get_signal_score(row):
        ...
        Score Range: -10 to +10 (approx)
        Enhanced with additional indicators
        """
        score = 0
        ...
        if row['close'] > row['ema_20'] > row['ema_50']:
            ...
            score += 0.5  # Mild uptrend
        elif row['close'] < row['ema_20']:
            score -= 0.5  # Mild downtrend
            
        # RSI (Oversold/Overbought)
        if row['rsi'] < 30:
            score += 1.5  # Oversold (Buy signal)
        elif row['rsi'] > 70:
            score -= 1.5  # Overbought (Sell signal)
        elif row['rsi'] < 40:
            score += 0.5  # Approaching oversold
        elif row['rsi'] > 60:
            score -= 0.5  # Approaching overbought
            
        # MACD Cross and Momentum
        ...
        # Bollinger Bands
        if 'bb_pct' in row:
            if row['bb_pct'] < 0.05:  # Near lower band
                score += 1.0  # Potential bounce
            elif row['bb_pct'] > 0.95:  # Near upper band
                score -= 1.0  # Potential reversal
        
        # Bollinger Band Width (Volatility)
        if 'bb_width' in row:
            if row['bb_width'] < 0.02:  # Low volatility, potential breakout
                score += 0.5
                
        # OBV Trend
        if 'obv' in row and 'obv_ema' in row and row['obv'] != 0:
            if row['obv'] > row['obv_ema']:
                score += 0.5  # Volume confirming uptrend
            elif row['obv'] < row['obv_ema']:
                score -= 0.5  # Volume confirming downtrend
